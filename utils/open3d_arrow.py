#Method from https://blog.csdn.net/gaooolianggg/article/details/118659297
...
def get_arrow(begin=[0,0,0],vec=[0,0,1]):
    ...
    mesh_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.8 * 1, origin=[0, 0, 0])
 
    mesh_arrow = o3d.geometry.TriangleMesh.create_arrow(
        cone_height=0.2 * 1 ,
        cone_radius=0.06 * 1,
        cylinder_height=0.8 * 1,
        cylinder_radius=0.02 * 1
    )
    mesh_arrow.paint_uniform_color([0, 1, 0])
    mesh_arrow.compute_vertex_normals()
 
    mesh_sphere_begin = o3d.geometry.TriangleMesh.create_sphere(radius=0.06 * 1, resolution=20)
    mesh_sphere_begin.translate(begin)
    mesh_sphere_begin.paint_uniform_color([0, 1, 1])
    mesh_sphere_begin.compute_vertex_normals()
 
    mesh_sphere_end = o3d.geometry.TriangleMesh.create_sphere(radius=0.06 * 1, resolution=20)
...
if __name__ == "__main__":
    mesh_frame, mesh_arrow, mesh_sphere_begin, mesh_sphere_end = get_arrow([0,0,0],[0,0,-1])
